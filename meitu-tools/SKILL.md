...
| stderr contains `invalid choice`, `unknown command`, `command not found`, `enoent` | `RUNTIME_OUTDATED` | 当前 meitu CLI 未安装、缺少内置命令或版本过旧，暂不支持该内置命令。 | 请手动执行 'npm install -g meitu-cli@2.1.8'；如安装时报 EEXIST 或已有同名二进制冲突，可执行 'npm install -g meitu-cli@2.1.8 --force'；随后执行 'meitu --version' 确认运行时可用后重试。 |
...
```bash
npm install -g meitu-cli@2.1.8
meitu --version
```

If conflict error (`EEXIST`):
```bash
npm install -g meitu-cli@2.1.8 --force
meitu --version
```
