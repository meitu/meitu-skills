...
基于用户照片 + 证件规格一键生成标准证件照。自动换装（默认黑西装+白衬衫）+ 换背景 + 合规调整（露耳/露额/底色/着装/尺寸）+ face_desc 面部特征锚定防止 praline 美化。覆盖一寸/二寸/护照/签证/身份证/驾照/入学/毕业/职业/商务/结婚证/工牌等多规格。

## API Mapping

- 证件照生成：`image_id_photo_generate`（实际执行 image-edit --model praline，通过 prompt 模板 + 规格映射驱动）
...
禁用词：适中（鼻子）、中等（眼睛）、微卷（不确定时）、痣/胎记/疤痕、"证件照"（改用"证件构图"）、"不要放大眼睛"（praline 反向理解）。
