...
| `gummy_pro` | `image_gummy_edit_v45` | 人像编辑（人脸 / 表情 / 妆容 / 发型 / 肤色 / 姿态 / 美颜） |
...
> 旧版 `--model nougat / gummy / praline` 在 2.0.x **不再被接受**。风格化（卡通/3D/动漫/水墨）改走 `image-style-transfer`。
...
把已有图风格化（卡通 / 3D / 动漫 / 水墨 / 油画 / 像素 等）。**取代旧版 image-edit + nougat 的用法。**
...
| Stylization（变画风） | `image-style-transfer --image_url "ref" --prompt "..."` | 旧版 image-edit + nougat 已废弃 |
