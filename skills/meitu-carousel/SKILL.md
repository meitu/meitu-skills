...
| 阶段 5a: 首张内页 | 参考封面风格 | 传入封面图 URL | 确保内页与封面视觉一致 |
| 阶段 5b: 后续内页 | 套用模板 | 传入模板页 URL | 复制模板页的排版和风格 |
...
| L5 | 停止并报错，向用户展示 JSON 输出中的 `error_code` 和 `user_hint`，建议调整需求或检查网络/凭证。若 `error_type` 为 ORDER_REQUIRED → 提示充值；CREDENTIALS_MISSING → 提示运行 `meitu config set-ak` + `meitu config set-sk` 配置凭证 |
