...
L5: 首步即失败 → 检查凭证/余额，报错含 error_code + user_hint
```
...
output_dir 已在 Preflight 解析完毕，文件已由 `--download-dir` 下载到 `{output_dir}`。Deliver 只做重命名：

`node $OC_SCRIPT rename --file {downloaded_file} --name {spec_name}` → 规范文件名

脚本不存在 →

```bash
mv "{output_dir}/{task_id_filename}" "{output_dir}/{date}_{spec_name}_{color_name}.jpg"
```

命名示例：`20260323_二寸_蓝底.jpg`、`20260323_美国护照_白底.jpg`
