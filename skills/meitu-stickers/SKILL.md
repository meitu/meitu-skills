...
```bash
meitu image-grid-split --image_url "<grid_image_url>" --download-dir {output_dir}/split   --skill_name meitu-stickers
```
...
```bash
meitu video-to-gif --video_url "{output_dir}/video/<video_task_id_file>" --prompt "transparent animated sticker" --json --download-dir {output_dir}/gif   --skill_name meitu-stickers
```
