...
> **说明：** 当前 CLI `2.0.6` 不再对外暴露 `image-edit --model nougat` 这类风格化入口，贴纸场景统一改为 `text-to-image --image_list`，通过参考图 + prompt 控制风格。
...
```bash
meitu image-grid-split --image_url "<grid_image_url>" --download-dir {output_dir}/split
```
...
```bash
meitu video-to-gif --video_url "{output_dir}/video/<video_task_id_file>" --prompt "transparent animated sticker" --json --download-dir {output_dir}/gif
```
