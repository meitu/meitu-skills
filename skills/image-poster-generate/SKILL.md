...
4. output_dir：openclaw.yaml → `./output/` ｜else → `$VISUAL/output/image-poster-generate/`；`mkdir -p`
...
```bash
meitu image-poster-generate --prompt "<desc>" [--model Praline_2] [--ratio 9:16] [--size auto] [--image_list <url>] --json --download-dir {output_dir} --skill_name skill_image-poster-generate
```
...
- `mv {downloaded_files[0].saved_path} {output_dir}/{YYYY-MM-DD}_{descriptive}_image-poster-generate.{ext}`
